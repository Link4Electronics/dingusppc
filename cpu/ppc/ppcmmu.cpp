    ...
    if (rgn_desc) {
        if (rgn_desc->type & RT_MMIO) {
            if (rgn_desc->devobj && rgn_desc->devobj->supports_type(HWCompType::ROM)) {
                LOG_F(WARNING, "ITLB: ROM write enabled during exec at 0x%08X, disabling", phys_addr);
            }
            if (!rgn_desc->mem_ptr) {
